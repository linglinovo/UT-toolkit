...
def click_button_5_actions():
    ...
    def click_5_actions():
        ...
        if os.path.exists(utfliepath):
            ...
            combobox["value"] = (
            "初始(0)", "第一个存档点(2)", "下一个房间按钮开门后(3)", "扳下一个switch后(4.5)", "两个switch后(5)",
            "攻击dummy前(5.5)", "解决dummy后(7)", "牵手过桥后(8)", "刚被扔下后(9)", "第二个存档点（已接到电话）(9.2)",
            "接到问口味电话后(9.4）", "接到确认口味电话后（9.6）", "确认过敏源后(9.8)", "小幽灵战后(11)",
            "接到提醒Inventory电话后(12)", "六块掉落地板解密后(13)", "第一个三色开关解谜后(14)",
            "第二个三色开关解谜后(15)", "第二个三色开关解谜后(16)", "到家门口(17)", "进门与Toriel对话一次后(18)",
            "介绍房间后(19)", "提过一次要回家后(19.4)", "Toriel去地下室后(21)", "地下室对话1次后(22)",
            ...
            "The_End存档点旁对话后(208)", "Epilogue(999)")
            combobox.current(0)
            def actions1():
                flag = False
                if RSlib.checktask("UNDERTALE.exe"):
    ...
    _button6 = tk.Button(child_window, text="编辑存档信息文件",width=15, command=lambda:click_6_actions(),font=("微软雅黑", 15))
