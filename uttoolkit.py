...
icon_data = ("iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAMAAAAoLQ9TAAADAFBMVEUBAACAAAAAgACAgAAAAI"
             "CAAIAAgICAgIDAwMD/AAAA/wD//wAAAP//AP8A//////8AAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
             ...
             "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
             "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
             "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAABMJ8z+A"
             "AAAAXRSTlMAQObYZgAAADpJREFUeNq1zDsKADAMAlCz9v5ndS7GfiEd66I8SAJoAKHkCJXFI7IkYzxh5Q8"
             "AJ7GETfRJASbOpzd0CFEMYeFyJNMAAAAASUVORK5CYII=")
RSlib.doadmin()  # 获取管理员
...
button1 = tk.Button(
    ...
    font=(
        "微软雅黑",
        14),
    command=lambda: click_button_1_actions())
...
button2 = tk.Button(
    ...
    font=(
        "微软雅黑",
        14),
    command=lambda: click_button_2_actions())
button2.pack(pady=10)
button3 = tk.Button(window, text="重命名角色", width=15, font=(
    "微软雅黑", 14), command=lambda: click_button_4_actions())
button3.pack(pady=10)
button4 = tk.Button(window, text="删除存档", width=15, font=(
    "微软雅黑", 14), command=lambda: click_button_3_actions())
button4.pack(pady=10)
button5 = tk.Button(window, text="其他", width=15, font=(
    "微软雅黑", 14), command=lambda: click_button_5_actions())
...
def click_button_1_actions():
    ...
    if os.path.exists(utfliepath):
        ...
        os.rename(
            ...
            ".utfile")
        shutil.move(
            a + "/Local/" + zipedutfliename + ".utfile",
            tk.filedialog.askdirectory(
                title="选择一个保存位置"))  # 获取保存位置
        messagebox.showinfo(
            "提示",
            "存档备份成功\n保存的文件名：" +
            zipedutfliename +
...
def click_button_2_actions():
    ...
    # 获取存档备份位置
    doloadedutflie = tk.filedialog.askopenfilename(
        title="选择你的存档文件", filetypes=[
            ('UT存档备份', '*.utfile'), ('UT存档zip备份', '*.zip')])
    # 判断是否为存档备份文件
    ...
    if os.path.exists(utfliepath + "/Packaged_using_UT-toolkit.LLXL"):
        os.remove(utfliepath + "/Packaged_using_UT-toolkit.LLXL")
    if checkfile1.is_file() and checkfile2.is_file(
    ) and os.path.exists("./cache/undertale.ini"):
        flag1 = True
    else:
        flag1 = False
    # 判断是否为存档备份文件
...
def click_button_5_actions():
    ...
    def click_0_actions():
        ...
        child_window3.resizable(False, False)
        # 子窗口
        ...
        def actions():
            ...
            if combobox.get() == "进入交易（即进入游戏与chara进行交易）":
                if tk.messagebox.askyesno("警告", "此操作会抹掉你的存档"):
                    ...
                    file = open(
                        utfliepath +
                        "/system_information_962.tmp",
                        'w')  # 创建文件
                    file.write("a")  # 写入
                    file.close()
                    os.rename(
                        utfliepath + "/system_information_962.tmp",
                        utfliepath + "/system_information_962")  # 重命名
    ...
    def click_2_actions():
        ...
        try:
            ...
            messageboxget = tk.messagebox.askyesnocancel("询问", "是否修改LOVE?")
            if messageboxget:
    ...
    def click_3_actions():
        ...
        try:
            tk.messagebox.showinfo("提示", "修改非实时生效，需进入游戏再次存档即可")
            tk.messagebox.showwarning(
                "警告", "在没有关闭dogcheck的情况下，进入0-4、239-263、及265-335会进入坏档狗的界面")
            # 子窗口
            child_window = tk.Toplevel(window)
            ...
            def act2():
                webbrowser.open(
                    "https://pcy.ulyssis.be/undertale/rooms")  # 打开网页
    ...
    def click_4_actions():
        ...
        child_window2.resizable(False, False)
        # 子窗口
        # 标题、输入框、选择框
        label2 = tk.Label(
            child_window2,
        ...
        # 一些按钮
        button = tk.Button(
            child_window2, text="确定", width=15, font=(
                "微软雅黑", 10), command=actions)
    ...
    def click_5_actions():
        ...
        try:
            ...
            combobox["value"] = (  # 选择框里的东西（剧情节点）
                ...
                "雪镇第一次能存档时(37)", "再见过Papyrus后(40)", "过了Doggo后(42)", "电击迷宫剧情后(43)",
                "crossword后(47)",
                "按下隐藏开关后(49)", "遇到狗夫妇后(51)", "解决一个XOXO谜题后(53)", "解决两个后(56)", "彩砖迷题后(58)",
                "滑冰谜题后(63)", "GreaterDog战后(65)", "刚进雪镇(67)", "Papyrus战后(101)",
                "Undyne初过场动画结束后(106)",
                ...
                try:
                    ...
                    if 0 <= input_num <= 100:  # 判断大小是否在合法区间
                        ...
                        with open(utfliepath + "/undertale.ini", "r", errors='ignore') as file:
                            lines = file.readlines()
                        rewirteline = [index for index, line in enumerate(
                            lines) if "fun" in line]  # 获取存档文件里要写入的位置
    ...
    def click_7_actions():
        ...
        label3 = tk.Label(
            child_window3,
            text="提示: won=0 杀死Asgore后会触发Omega Flowey战，大于0则不会",
            fg="black",
            font=(
                "微软雅黑",
                10))
            ...
            try:
                ...
                with open(utfliepath + "/undertale.ini", 'r', errors='ignore') as file:
                    ...
                    if lines_target:
                        ...
                        with open(utfliepath + "/undertale.ini", 'a', errors='ignore') as file:
                            file.write(text + '\n')
                tk.messagebox.showinfo("提示", "修改完成")
                if flag:
                    os.startfile(uttaskpath)
    ...
    _button0 = tk.Button(
        ...
        text="创建屠杀线残留文件",
        width=15,
        command=lambda: click_0_actions(),
        font=(
            "微软雅黑",
            15))
    ...
    _button1 = tk.Button(
        ...
        text="删除屠杀线残留文件",
        width=15,
        command=lambda: click_1_actions(),
        font=(
            "微软雅黑",
            15))
    ...
    _button2 = tk.Button(
        ...
        text="一键神装",
        width=15,
        command=lambda: click_2_actions(),
        font=(
            "微软雅黑",
            15))
    ...
    _button3 = tk.Button(
        ...
        text="修改room",
        width=15,
        command=lambda: click_3_actions(),
        font=(
            "微软雅黑",
            15))
    ...
    _button4 = tk.Button(
        ...
        text="获取物品",
        width=15,
        command=lambda: click_4_actions(),
        font=(
            "微软雅黑",
            15))
    ...
    _button5 = tk.Button(
        ...
        text="修改剧情",
        width=15,
        command=lambda: click_5_actions(),
        font=(
            "微软雅黑",
            15))
    ...
    _button6 = tk.Button(
        ...
        text="修改fun",
        width=15,
        command=lambda: click_6_actions(),
        font=(
            "微软雅黑",
            15))
    ...
    _button7 = tk.Button(
        ...
        text="设定won",
        width=15,
        command=lambda: click_7_actions(),
        font=(
            "微软雅黑",
            15))
    ...
    _button8 = tk.Button(
        ...
        text="编辑存档信息文件",
        width=15,
        command=lambda: click_8_actions(),
        font=(
            "微软雅黑",
            15))
